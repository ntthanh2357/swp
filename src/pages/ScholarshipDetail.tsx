...
const ScholarshipDetail: React.FC = () => {
  ...
  // Mock scholarship data (in real app, fetch from API)
  const mockScholarships: Scholarship[] = [
    {
      ...
      description: 'Supporting diversity in technology through education. This comprehensive scholarship program aims to increase representation in STEM fields and provide opportunities for underrepresented students to pursue their academic goals in technology-related disciplines.',
  ...
  useEffect(() => {
    // Simulate API call
    const findScholarship = mockScholarships.find(s => s.id === id);
    setScholarship(findScholarship || null);
    
    // Check if saved (from localStorage for demo)
  ...
  const handleSaveScholarship = () => {
    ...
    if (isSaved) {
      // Remove from saved
      const updated = savedScholarships.filter((savedId: string) => savedId !== id);
      localStorage.setItem('savedScholarships', JSON.stringify(updated));
      setIsSaved(false);
    } else {
      // Add to saved
      const updated = [...savedScholarships, id];
      localStorage.setItem('savedScholarships', JSON.stringify(updated));
      setIsSaved(true);
            ...
            <div className="bg-white rounded-2xl shadow-sm p-6">
              ...
              <div className="space-y-4">
                ...
                >
                  ...
                  <span>{isSaved ? 'Remove from Saved' : 'Save Scholarship'}</span>
                </button>
